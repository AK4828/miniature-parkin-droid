...
public class SettleDatabaseAdapter extends DefaultDatabaseAdapter {
    ...
    public void saveSettles(List<Settle> settles) {
        for (Settle settle : settles) {
            ...
            if (findSettleByRefId(settle.getId()) == null) {
                ...
                productStoreDbAdapter.saveProductStore(settle.getProduct());
                ...
                contentValues.put(SettleDatabaseModel.REF_ID, settle.getId());
                ...
                context.getContentResolver().update(dbUriSettle, contentValues,
                        SettleDatabaseModel.REF_ID + " = ?", new String[] { settle.getId() });
            }
        }
    }
    ...
    public List<Settle> findAllSettle(String assigmentDetailId) {
        String criteria = SettleDatabaseModel.STATUS_FLAG + " = ? AND "
                + SettleDatabaseModel.ASSIGMENT_DETAIL_ID + " = ?";
        ...
        if (cursor != null) {
            while (cursor.moveToNext()) {
                ...
                productStore = productStoreDbAdapter.findProductStoreById(cursor.getString
                        (cursor.getColumnIndex(SettleDatabaseModel.PRODUCT_ID)));

                settle.setId(cursor.getString(cursor.getColumnIndex(SettleDatabaseModel.ID)));
                settle.getAssigmentDetail().setId(cursor.getString(cursor.getColumnIndex
                        (SettleDatabaseModel.ASSIGMENT_DETAIL_ID)));
                settle.setProduct(productStore);

...
public class AssigmentDetailDatabaseAdapter extends DefaultDatabaseAdapter {
    ...
    public AssigmentDetailDatabaseAdapter(Context context) {
        ...
        userDbAdapter = new UserDatabaseAdapter(context);
    }
    ...
    public void saveAssigmentDetails(List<AssigmentDetail> assigmentDetails) {
        for (AssigmentDetail assigmentDetail : assigmentDetails) {
            ...
            if (findAssigmentDetailByRefId(assigmentDetail.getId()) == null) {
                ...
                contentValues.put(AssigmentDetailDatabaseModel.STATUS, assigmentDetail.getStatus().name());
                contentValues.put(AssigmentDetailDatabaseModel.REF_ID, assigmentDetail.getId());

                contentValues.put(DefaultPersistenceModel.SYNC_STATUS, 0);
                contentValues.put(DefaultPersistenceModel.STATUS_FLAG, 1);

                userDbAdapter.saveUser(assigmentDetail.getAgent());
                ...
                context.getContentResolver().update(dbUriAssigmentDetail, contentValues,
                        AssigmentDetailDatabaseModel.REF_ID + " = ?", new String[] { assigmentDetail.getId() });
            }
        }
    }
    ...
    public AssigmentDetail findAssigmentDetailByRefId(String refId) {
        ...
        if (cursor != null) {
            if (cursor.getCount() > 0) {
                ...
                assigmentDetail.setStatus(AssigmentDetail.AssigmentDetailStatus.valueOf
                        (cursor.getString(cursor.getColumnIndex(AssigmentDetailDatabaseModel.STATUS))));
            }
        }
    ...
    public List<AssigmentDetail> findAllAssigmentDetail(String assigmentId) {
        String criteria = AssigmentDetailDatabaseModel.STATUS_FLAG + " = ? AND "
                + AssigmentDetailDatabaseModel.ASSIGMENT_ID + " = ? ";
        ...
        if (cursor != null) {
            while (cursor.moveToNext()) {
                ...
                User agent = userDbAdapter.findUserByRefId(cursor.getString(cursor.getColumnIndex
                        (AssigmentDetailDatabaseModel.AGENT_ID)));

                assigmentDetail.setId(cursor.getString(cursor.getColumnIndex(AssigmentDetailDatabaseModel.ID)));
                assigmentDetail.setAgent(agent);
    ...
    public List<AssigmentDetail> findAssigmentDetailSettle(String assigmentId, String status) {
        String criteria = AssigmentDetailDatabaseModel.STATUS_FLAG + " = ? AND "
                + AssigmentDetailDatabaseModel.ASSIGMENT_ID + " = ? AND "
                + AssigmentDetailDatabaseModel.STATUS + " = ? ";

        String[] parameter = { SignageVariables.ACTIVE, assigmentId, status };
    ...
    public void updateSyncStatusById(String refId) {
        ...
        context.getContentResolver().update(dbUriAssigmentDetail, values,
                AssigmentDetailDatabaseModel.REF_ID + " = ? ", new String[]{refId});
    }

    public void updateStatusByRefId(String refId, AssigmentDetail.AssigmentDetailStatus detailStatus) {
        Log.d(getClass().getSimpleName(), "updateStatusAsgById.Success2");
        ContentValues values = new ContentValues();
        values.put(AssigmentDetailDatabaseModel.STATUS, detailStatus.name());
    ...
    public void deleteByRefIds(List<String> refIds) {
        for(String id : refIds) {
            ...
            context.getContentResolver().update(dbUriAssigmentDetail, values,
                    AssigmentDetailDatabaseModel.REF_ID + " = ? ", new String[] { id });
        }
    }
